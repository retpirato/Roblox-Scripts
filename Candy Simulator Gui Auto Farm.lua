...
TextLabel.Text = "| Clicker, Seller, TP Candy/Coin | by reav"
...
TextLabel_2.Text = "Candy Simulator"
...
while wait() do 
	if ok then
		wait(.5)
		mouse1click()
		workspace.Pads.Sell2x.SELL.CFrame=game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame+Vector3.new(0,0,0)
		...
		for i,v in pairs(workspace.Candys:GetChildren()) do 
			if v.ClassName == "MeshPart" and v:FindFirstChild("TouchInterest") then 
				v.CFrame=game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame+Vector3.new(0,0,-1)
			end
		end
	end
end
