...
function kill()
	while modifier do
		wait()
		if game.Workspace.Enemies["Frost Guard"]:FindFirstChild(" ") ~= nil then
			if game.Workspace.Enemies["Frost Guard"][" "]:FindFirstChild("UpperTorso") ~= nil then
				if game.Workspace.Enemies["Frost Guard"][" "]:FindFirstChild("HumanoidRootPart") ~= nil then
					hum = game.Players.LocalPlayer.Character.HumanoidRootPart
					enemie = game.Workspace.Enemies["Frost Guard"]:FindFirstChild(" ")
					hum.CFrame = enemie.HumanoidRootPart.CFrame * CFrame.new(-1,0,3)
					if enemie:FindFirstChild("FrostGuardSword") ~= nil then
						sword = enemie:FindFirstChild("FrostGuardSword")
						sword:Destroy()
					end
					...
					if enemie.Humanoid.Health ~= enemie.Humanoid.MaxHealth and game.Workspace.Enemies:FindFirstChild("Frost Guard") ~= nil then
